...
// reload changes (see build task)
// Example:
//    gulp --reload
var isReload = !!args.reload;

if (isProduction)
    log('Starting production build...');

if (isReload)
    log('Reloading when changes occurs...');
...
gulp.task('scripts:app', function() {
    ...
    return gulp.src(source.scripts.entry)
        ...
        .pipe( $.if((isProduction || isReload), reload({ stream: true})) );
});
...
gulp.task('clean:images', function (cb) {
    ...
    return del(build.images,cb);*/
    log('Clean dist/img folder..');
...
gulp.task('watch', function() {
    ...
    gulp.watch(source.images, ['clean:images']);
    gulp.watch(source.images, ['images']);
